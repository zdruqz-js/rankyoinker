...
# ============================ Patch-Hinweis ============================
# Originalquelle von vRY 2.94 (src/websocket.py) mit zwei Ergaenzungen:
#
# 1. Zustandswechsel-Fallback: Normalerweise erfaehrt vRY ueber den Chat-
#    Websocket, wenn man ins Agent Select oder ins Spiel wechselt. Laeuft
#    Deceive ("offline erscheinen"), kommt die *eigene* Presence nie an -
#    vRY wartet dann ewig und aktualisiert die Spielerliste nie.
#    Parallel zum Websocket wird deshalb alle paar Sekunden der Zustand
#    ueber die Spiel-API geprueft, aber nur solange die eigene Presence
#    tatsaechlich fehlt. Ist alles normal, aendert sich nichts.
#
# 2. Leere Presence-Eintraege werden in handle() uebersprungen, statt bei
#    jedem Event "Failed to decode private presence data" zu loggen.
#
# Rueckgaengig machen = diese Datei loeschen (websocket.pyc bleibt intakt).
# =======================================================================

# Wie oft der Fallback den Zustand ueber die Spiel-API prueft (Sekunden)
...
class Ws:
    def __init__(self, lockfile, Requests, cfg, colors, hide_names, server, rpc=None):
        ...
        if self.cfg.get_feature_flag("discord_rpc"):
            self.rpc = rpc
        # fuer den Zustands-Fallback (siehe Patch-Hinweis)
        self._presences = Presences(Requests, self.log)
        self._last_rpc_refresh = 0.0
        ...
        for attempt in range(max_retries):
            ...
            try:
                async with websockets.connect(url, ssl=self.ssl_context, extra_headers=local_headers) as websocket:
                    ...
                    # laeuft parallel mit und greift nur, wenn die eigene
                    # Presence fehlt (Deceive & Co.)
                    poll_task = asyncio.ensure_future(self._poll_game_state(initial_game_state))
                ...
                for task in (recv_task, poll_task):
    ...
    async def _poll_game_state(self, initial_game_state):
        """Fallback: Zustandswechsel ueber die Spiel-API erkennen.

        Gibt erst zurueck, wenn sich der Zustand wirklich geaendert hat -
        solange die eigene Chat-Presence normal ankommt, tut die Schleife
    ...
    def _detect_state_via_api(self, initial_game_state):
        ...
        state = self._presences.get_api_game_state(prefer=prefer, max_age=0)

        now = time.time()
        if state != initial_game_state or now - self._last_rpc_refresh > RPC_REFRESH_INTERVAL:
            self._last_rpc_refresh = now
            self._refresh_rpc()
        return state

    def _refresh_rpc(self):
        """Discord-Status aktuell halten, obwohl keine Presence-Events kommen."""
        if not self.cfg.get_feature_flag("discord_rpc"):
            return
        rpc = getattr(self, "rpc", None)
        if rpc is None:
            return
        try:
            private_presence = self._presences.get_private_presence(self._presences.get_presence())
            if private_presence:
                rpc.set_rpc(private_presence)
        except Exception as e:
            self.log(f"Discord-Status konnte nicht aktualisiert werden: {e}")
    ...
    def handle(self, m, initial_game_state):
        ...
        if resp_json[2].get("uri") == "/chat/v4/presences":
            ...
            if presence.get('puuid') == self.Requests.puuid:
                ...
                # leere Huelle (z. B. der riot_client-Eintrag, den Deceive
                # uebrig laesst) - nichts zu dekodieren
